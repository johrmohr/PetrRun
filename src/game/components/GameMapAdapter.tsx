...
// Helper function to convert pixel coordinates to geographic coordinates (0-1) to geographic coordinates
function normalizedToGeo(x: number, y: number): { lat: number; lng: number } {
  const minLat = 40.705;
  const maxLat = 40.72;
  const minLng = -74.015;
  const maxLng = -73.995;
  ...
  const lat = maxLat - y * (maxLat - minLat); // Flip Y back
